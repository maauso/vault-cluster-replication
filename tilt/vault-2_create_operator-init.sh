...
if [ "$sealed" = "true" ]; then
  # Init Vault cluster
  vault operator init -key-shares=1 -key-threshold=1 -format=json > vault-2_unseal_keys.json
fi

# Unseal Vault cluster
vault operator unseal $(cat ./vault-2_unseal_keys.json | jq -r ".unseal_keys_b64[]")

export VAULT_TOKEN=$(jq -r '.root_token' vault-2_unseal_keys.json)
